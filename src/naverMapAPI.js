    ...
    return (
      ...
      <NaverMap
        ...
        >
        {
            ...
            animation={0}
            onClick={() => {setLgShow(true); setIdx(i);}}
            icon={{
                content:
        ...
        >
          ...
          <Modal.Body>
          {
            data.current.map((a,i)=>
              ...
              ?<>
                <h5 className="mt-3 mb-5"><b>{data.current[i].noticeNo}</b></h5>
                <img alt="file" src={data.current[i].filename} className="mb-5"/>
                {
                  data.current[i].sexCd === 'M'
                  ? <h6 className="mb-3">품종 : {data.current[i].kindCd} / 성별 : 남</h6>
                  : <h6 className="mb-3">품종 : {data.current[i].kindCd} / 성별 : 여</h6>
                }
                <h6 className="mb-3">털색 : {data.current[i].colorCd} / 체중 : {data.current[i].weight}</h6>
                ...
                <h6 className="mb-3">공고 : {data.current[i].noticeSdt}~{data.current[i].noticeEdt}</h6>
